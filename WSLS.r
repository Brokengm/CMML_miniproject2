...
type <- function(risk, Schema_res, Item_EI, Outputs_cho, ThisPhase, schemainfo){
...
simulation <- function(Param.df, type, exp_type, save=F, savepath="",
  ...
  last_schema_vec <- c(NA,NA) #change:define two new variables to store the schema and result(success or not) in the last round
  last_success_vec <- c(FALSE,FALSE) 
  ...
  for (Subject in 1:subjectnum) {
    ...
    while (clock < 4500){# new clock < 4500 i.e. total time is (85-10)*60 seconds
      ...
      if (break_happens == 0 & clock >= 1500){
        ...
        if (type %in% c("L","H")){
          schemainfo2 <- expschema %>% filter(type=='new') %>% select(-c(author,new)) #只加type=new的
      ...
      while (decisiontime <= 2){ ## i.e. 2 phase to choose item
        ...
        #change:
        # --- WSLS：Win–Stay / Lose–Shift in Choschema---
        # --- WSLS Filtering using Previous Round's Memory ---
        if (!all(is.na(last_schema_vec))) {
            # Determine which schema were successful and which were not
            win_schemas <- last_schema_vec[last_success_vec]    # those rounds that were wins
            lose_schemas <- last_schema_vec[!last_success_vec]    # those rounds that were losses
            
            if (length(win_schemas) == 2) {
                # Both decisions in last round were wins: candidate set are both schemas.
                candidate <- unique(win_schemas)
                cat("WSLS: Both rounds succeeded, retaining schemas:", candidate, "\n")
                ChoSchema <- ChoSchema[ChoSchema$schemaID %in% candidate, ]
            } else if (length(win_schemas) == 1) {
                # Only one decision in last round was a win: candidate set is the successful one.
                candidate <- win_schemas
                cat("WSLS: One round succeeded, retaining schema:", candidate, "\n")
                ChoSchema <- ChoSchema[ChoSchema$schemaID %in% candidate, ]
            } else {
                # Both rounds failed: exclude both candidate schemas.
                candidate <- lose_schemas
                cat("WSLS: Both rounds failed, excluding schemas:", candidate, "\n")
                ChoSchema <- ChoSchema[!ChoSchema$schemaID %in% candidate, ]
            }
            
            # (Optional) If filtering results in an empty candidate set, revert to the unfiltered ChoSchema.
            if(nrow(ChoSchema) == 0) {
                cat("WSLS: ChoSchema is empty after filtering; reverting to original candidate set.\n")
                if (decisiontime == 1) {
                ChoSchema <- Schema_EI[Schema_EI$schemaID %in% Thischosen$chosen, ]
                } else {
                ChoSchema <- Schema_EI[!Schema_EI$schemaID %in% Thischosen$chosen, ]
        ...
        ChoItem <- Decitem %>%
          filter(SchemaID %in% ChoSchema$schemaID) %>%
          group_by(SchemaID) %>%
          slice_sample(n = 4) %>%
          ungroup() %>%
          select(Schema = SchemaID, chosen = ItemID)
        ...
        Item_EI = Item_EI %>% 
          ...
          mutate(N = schemainfo$conN[schemainfo$schemaID == Schema], 
                 ...
                 DM = first(ChoSchema$DM[ChoSchema$schemaID == Schema]))
          ...
          if (attention == 0) { ### i.e., when decision has not been made and no item were attended
            ...
            if (sum(Item_EI$decision == 0) == 1){ 
              ...
              if (schema_decision == 0){
                ...
                if (length(Item_EI$decision) > 0 && all(Item_EI$decision == 1, na.rm = TRUE)) {
              ...
              else if (length(Item_EI$ID[Item_EI$decision == 0])==0) {
                break
              }
        ...
        feedback = type(risk, Schema_res, Item_EI, Outputs_cho, ThisPhase, schemainfo)
        ...
        last_schema_vec[2] <- Outputs_cho$Schema[ThisPhase]
        last_success_vec[2] <- (Outputs_cho$payoff[ThisPhase] >= 5 && Outputs_cho$AC[ThisPhase] == 1)
        }
...
res <- simulation(Param.df, "L", "painting", save = T,
                  savepath = "res100_Q2_wsls_tight2/tmp", sim.mode="before",
                  before.path = NA,save.confi = T,
                  scale.confi.init = T)

res <- simulation(Param.df, "L", "painting", save = T,
                  savepath = "res100_parameter_modified/tmp/after_tmp", sim.mode="after",
                  before.path = file.path("res100_parameter_modified/tmp", "L"),
                  scale.confi.init = T)
